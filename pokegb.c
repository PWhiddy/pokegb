...
static inline uint8_t mem8(uint16_t addr, uint8_t val, int write);
static inline uint8_t reg8_access(uint8_t val, int write, uint8_t o);
static inline uint16_t read16(uint16_t *addr);
static inline void push(uint16_t val);
static inline void set_flags(uint8_t mask, int Z, int N, int H, int C);
...
#define OP5_FLAG(_, always) OP4_NX8(_, 1) case always: OPCREL(_), carry = opcode == always || !(*F & F_mask[opcrel]) ^ (opcrel & 1);
#define OP8_REL(_) case _ ... _ + 7: tmp8 = reg8_access(0, 0, opcrel = opcode);
#define OP8_NX8_REL(_) OP4_NX8(_, 1) OP4_NX8(_ + 32, 1) tmp8 = reg8_access(0, 0, OPCREL(_));
#define OP64_REL(_) case _ ... _ + 55: OP8_REL(_ + 56) OPCREL(_);
#define OP9_IMM_PTR(_) OP8_REL(_) case _ + 70 : operand = opcode & 64 ? mem8(PC++, 0, 0) : tmp8;
...
int last_frame_count_log = 0; 
int uncoupled_mode = 0; 
int render_frame = 1;   

/* --- Implementation --- */

static inline void tick() { cycles += 4; }
  ...
  switch (addr >> 13) {
    ...
    case 7:
      if (addr >= 65024) {
        ...
        if (addr == 65280) {
          if (~io[256] & 16) return ~(16 + key_state[SDL_SCANCODE_DOWN] * 8 + key_state[SDL_SCANCODE_UP] * 4 + key_state[SDL_SCANCODE_LEFT] * 2 + key_state[SDL_SCANCODE_RIGHT]);
          if (~io[256] & 32) return ~(32 + key_state[SDL_SCANCODE_RETURN] * 8 + key_state[SDL_SCANCODE_TAB] * 4 + key_state[SDL_SCANCODE_Z] * 2 + key_state[SDL_SCANCODE_X]);
          return 255;
        }
        return io[addr & 511];
      }
...
static inline uint8_t get_color(int tile, int y_offset, int x_offset) {
  uint8_t *tile_data = &video_ram[tile * 16 + y_offset * 2];
  return (tile_data[1] >> x_offset) % 2 * 2 + (*tile_data >> x_offset) % 2;
}

int main() {
  reg16 = (uint16_t *)reg8; HL = &reg16[2]; F = &reg8[6]; A = &reg8[7];
  reg8_group[0] = &reg8[1]; reg8_group[1] = &reg8[0]; reg8_group[2] = &reg8[3]; reg8_group[3] = &reg8[2];
  reg8_group[4] = &reg8[5]; reg8_group[5] = &reg8[4]; reg8_group[6] = F; reg8_group[7] = A;
  ...
  if (rom_fd < 0) { printf("Error: Could not open rom.gb\n"); return 1; }
  ...
  *LCDC = 145; *DIV = 44032;
  ...
  last_log_time = last_render_time = SDL_GetTicks();

  while (1) {
    prev_cycles = cycles;
    if (IME && (*IF & io[511])) { *IF = halt = IME = 0; cycles += 8; push(PC); PC = 64; }
    else if (halt) { tick(); }
    else {
      switch (opcode = mem8(PC++, 0, 0)) {
        OP4_NX16_REL(1) *reg16_group1[opcrel] = read16(&PC); break;
        case 0: break;
        OP4_NX16_REL(10) OP4_NX16_REL(2) tmp = opcode & 8; reg8_access(mem8(*reg16_group2[opcrel], *A, !tmp), tmp, 7); *HL += opcrel < 2 ? 0 : 5 - 2 * opcrel; break;
        ...
        OP9_IMM_PTR(152) carry = !(*F / 16 % 2); SUBTRACT: neg = 1; operand = ~operand; ALU: set_flags(0, tmp8 = *A + operand + carry, neg, (*A % 16 + operand % 16 + carry > 15) ^ neg, (*A + operand + carry > 255) ^ neg); if (~(opcode / 8) & 7) *A = tmp8; break;
        ...
        case 203: neg = 0; opcode = mem8(PC++, 0, 0); ROTATE: switch (opcode) {
            ...
            OP8_REL(8) OP8_REL(24) OP8_REL(40) OP8_REL(56) carry = tmp8 & 1; tmp8 = (opcode & 48) == 32 ? (int8_t)tmp8 >> 1 : tmp8 / 2 + (opcode & 32 ? 0 : opcode & 16 ? (*F * 8 & 128) : carry * 128); CARRY_ZERO_FLAGS_U: reg8_access(tmp8, 1, opcrel); set_flags(0, neg || tmp8, 0, 0, carry); break;
        ...
        case 224: case 226: case 234: case 240: case 242: case 250: tmp = opcode & 16; reg8_access(mem8(opcode & 8 ? read16(&PC) : 65280 + (opcode & 2 ? *reg8 : mem8(PC++, 0, 0)), *A, !tmp), tmp, 7); break;
    ...
    /* --- OPTIMIZED PPU LOGIC --- */
    /* Instead of looping 1-by-1 to catch up, we add all cycles at once. */
    int delta = cycles - prev_cycles;
    *DIV += delta;
    if (*LCDC & 128) {
        ppu_dot += delta;
        // Only run heavy logic if we crossed a scanline boundary
        while (ppu_dot >= 456) {
            ppu_dot -= 456;
            if (*LY < 144 && render_frame) {
                for (tmp = 160; --tmp >= 0;) {
            ...
            if (*LY == 143) {
                *IF |= 1;
                /* --- Logging / Events (Once per frame) --- */
                current_time = SDL_GetTicks();
                if (current_time - last_log_time >= 1000) {
                    int frames_this_second = frame_count_total - last_frame_count_log;
                    printf("Updates/Sec: %d | Speed: %.2fx\n", frames_this_second, frames_this_second / 60.0f);
                    last_frame_count_log = frame_count_total; last_log_time = current_time;
                }
                SDL_Event event;
                while (SDL_PollEvent(&event)) {
                    if (event.type == SDL_QUIT) return 0;
                    if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_u) {
                        uncoupled_mode = !uncoupled_mode;
                        printf("Mode: %s\n", uncoupled_mode ? "Uncoupled (Smart)" : "Fixed (128x)");
                    }
                }
                if (render_frame) {
            ...
            *LY = (*LY + 1) % 154;
            /* --- Render Scheduling Logic --- */
            if (*LY == 0) {
                frame_count_total++;
                if (uncoupled_mode) render_frame = (SDL_GetTicks() - last_render_time >= 16);
                else render_frame = (frame_count_total % 512 == 0);
            }
        }
    } else *LY = ppu_dot = 0;
  }
